  ...
  return (
    ...
    >
      <div className="text-center mb-12">
        <h2 className="font-display text-3xl md:text-4xl font-extrabold text-ink">About Me</h2>
        <p className="text-ink-muted mt-3">My background and technical expertise.</p>
      </div>
      ...
      <div className="flex flex-col gap-8">
        ...
        >
          <h3 className="font-display text-xl font-bold mb-6 text-ink">Background</h3>

          {/* Flexbox untuk mensejajarkan foto dan teks */}
          <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">

            {/* Wadah Foto */}
            <div className="w-48 h-48 md:w-64 md:h-64 shrink-0 rounded-2xl overflow-hidden border-4 border-surface ring-1 ring-line shadow-lg bg-surface-2">
        ...
        <motion.div 
          ...
          className="saas-card p-8 bg-surface-2 flex flex-col gap-6"
        >
          <div>
            <h3 className="font-display text-xl font-bold mb-1 text-ink">Skill Experience</h3>
            <p className="text-sm text-ink-subtle">My Skill, Based On My Experience.</p>
          </div>

          <div className="flex flex-col gap-6">
            {techArsenal.map((section, idx) => (
              <div key={idx}>
                <h4 className="text-xs font-bold text-accent uppercase tracking-widest mb-3 border-b border-line pb-2">
                  {section.category}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {section.skills.map((skill, index) => (
                    <span
                      key={index}
                      className="bg-surface border border-line text-ink-muted px-3 py-1.5 rounded-lg text-xs font-medium shadow-sm hover:border-accent hover:text-accent transition-colors cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
